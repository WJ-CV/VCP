...
class Cosal_Module(nn.Module):
    def __init__(self, H, W, out_C):
        super(Cosal_Module, self).__init__()
        self.cosal_feat = Cosal_Sub_Module(H, W)
        self.conv = nn.Sequential(nn.Conv2d(256, out_C, 1), Res(out_C))
...
class Cosal_Sub_Module(nn.Module):
    ...
    def forward(self, feats, SISMs):
        ...
        def get_co_maps(co_proxy, NFs):
            correlation_maps = F.conv2d(NFs, weight=co_proxy) 

            # Normalize correlation maps.
            correlation_maps = F.normalize(correlation_maps.reshape(N, N, HW), dim=2) 
            co_attention_maps = torch.sum(correlation_maps , dim=1)
            ...
            co_attention_maps = co_attention_maps.view(N, 1, H, W) 
        ...
        co_attention_maps = get_co_maps(co_proxy, NFs) 
        CoFs = get_CoFs(NFs, co_representation)
        co_saliency_feat = self.conv(CoFs * co_attention_maps)
...
class MixVisionTransformerVCP(nn.Module):
    ...
    def forward_features(self, x, train=True):
        B = x.shape[0]
        outs = []
        initial_prediction = []
        ...
        if '1' in self.tuning_stage:
            prompt1, initial_prediction1 = self.prompt_generator.init_prompt(x, handcrafted1, 1, H, W, train=train)
        ...
        if '2' in self.tuning_stage:
            prompt2, initial_prediction2 = self.prompt_generator.init_prompt(x, handcrafted2, 2, H, W, train=train)
        ...
        if '3' in self.tuning_stage:
            prompt3,  initial_prediction3 = self.prompt_generator.init_prompt(x, handcrafted3, 3, H, W, train=train)
        ...
        if '4' in self.tuning_stage:
            prompt4, initial_prediction4 = self.prompt_generator.init_prompt(x, handcrafted4, 4, H, W, train=train)
        ...
        outs.append(x)
        initial_prediction = [initial_prediction1, initial_prediction2, initial_prediction3, initial_prediction4]

        return outs, initial_prediction

    def forward(self, x, train=True):
        x, initial_prediction = self.forward_features(x, train=train)
        # x = self.head(x)

        return x, initial_prediction
...
class PromptGenerator(nn.Module):
    def __init__(self, scale_factor, prompt_type, embed_dims, tuning_stage, depths, input_type,
        ...
        self.embed_dims = embed_dims
        self.input_type = input_type   ### fft
        ...
        if self.embedding_tune:
            if '1' in self.tuning_stage:
                ...
                self.embedding_Co1 = Cosal_Module(72, 72, self.embed_dims[0] // self.scale_factor)
                self.embedding_prototype1 = prototype_w(self.embed_dims[0] // self.scale_factor, 35)
            if '2' in self.tuning_stage:
                self.embedding_generator2 = nn.Linear(self.embed_dims[1], self.embed_dims[1] // self.scale_factor)
                self.embedding_Co2 = Cosal_Module(36, 36, self.embed_dims[1] // self.scale_factor)
                self.embedding_prototype2 = prototype_w(self.embed_dims[1] // self.scale_factor, 35)
            if '3' in self.tuning_stage:
                self.embedding_generator3 = nn.Linear(self.embed_dims[2], self.embed_dims[2] // self.scale_factor)
                self.embedding_Co3 = Cosal_Module(18, 18, self.embed_dims[2] // self.scale_factor)
                self.embedding_prototype3 = prototype_w(self.embed_dims[2] // self.scale_factor, 35)
            if '4' in self.tuning_stage:
                self.embedding_generator4 = nn.Linear(self.embed_dims[3], self.embed_dims[3] // self.scale_factor)
                self.embedding_Co4 = Cosal_Module(9, 9, self.embed_dims[3] // self.scale_factor)
                self.embedding_prototype4 = prototype_w(self.embed_dims[3] // self.scale_factor, 35)

        if self.adaptor == 'adaptor':
            if '1' in self.tuning_stage:
                for i in range(self.depths[0]):
                    lightweight_mlp = nn.Sequential(
                            nn.Linear(2*self.embed_dims[0] // self.scale_factor, self.embed_dims[0] // self.scale_factor),
                            nn.GELU(),
                        )
            ...
            if '2' in self.tuning_stage:
                for i in range(self.depths[1]):
                    lightweight_mlp = nn.Sequential(
                            nn.Linear(2*self.embed_dims[1] // self.scale_factor, self.embed_dims[1] // self.scale_factor),
                            nn.GELU(),
                        )
            ...
            if '3' in self.tuning_stage:
                for i in range(self.depths[2]):
                    lightweight_mlp = nn.Sequential(
                            nn.Linear(2*self.embed_dims[2] // self.scale_factor, self.embed_dims[2] // self.scale_factor),
                            nn.GELU(),
                        )
            ...
            if '4' in self.tuning_stage:
                for i in range(self.depths[3]):
                    lightweight_mlp = nn.Sequential(
                            nn.Linear(2*self.embed_dims[3] // self.scale_factor, self.embed_dims[3] // self.scale_factor),
                            nn.GELU(),
                        )
    ...
    def init_prompt(self, embedding_feature, handcrafted_feature, block_num, H, W, train=True):
        if self.embedding_tune:
            embedding_generator = getattr(self, 'embedding_generator{}'.format(str(block_num)))

            embedding_Cosal = getattr(self, 'embedding_Co{}'.format(str(block_num)))
            embedding_prototype = getattr(self, 'embedding_prototype{}'.format(str(block_num)))

            embedding_feature = embedding_generator(embedding_feature)
            N, l, Channel = embedding_feature.size()

            if train == True :
                embedding_feature1 = embedding_feature[:N//3, :, :]
                embedding_feature2 = embedding_feature[N // 3: 2*N // 3, :, :]
                embedding_feature3 = embedding_feature[2*N // 3: , :, :]

                consensus_f1, feat_pro_prediction1 = self.gener_consensus_feature(embedding_feature1, embedding_Cosal, embedding_prototype, H, W)
                consensus_f2, feat_pro_prediction2 = self.gener_consensus_feature(embedding_feature2, embedding_Cosal, embedding_prototype, H, W)
                consensus_f3, feat_pro_prediction3 = self.gener_consensus_feature(embedding_feature3, embedding_Cosal, embedding_prototype, H, W)

                consensus_feature = torch.cat((consensus_f1, consensus_f2, consensus_f3), 0)
                initial_prediction = torch.cat((feat_pro_prediction1, feat_pro_prediction2, feat_pro_prediction3), 0)
            else:
                consensus_feature, initial_prediction = self.gener_consensus_feature(embedding_feature, embedding_Cosal, embedding_prototype, H, W)

        if self.handcrafted_tune:
            handcrafted_feature = handcrafted_feature

        return [handcrafted_feature, embedding_feature, consensus_feature], initial_prediction


    def gener_consensus_feature(self, embedding_feature, embedding_Cosal, embedding_prototype, H, W):
        N, l, C = embedding_feature.size()

        trans_embedding_feature = embedding_feature.permute(0, 2, 1).view(N, -1, H, W).contiguous()   # N, l, C -->>  N, C, H, W
        feat_pro_prediction = embedding_prototype(trans_embedding_feature)   # N, C, H, W  -->>  N, 1, H, W
        consensus_feature = embedding_Cosal(trans_embedding_feature, feat_pro_prediction)  # N, C, H, W

        consensus_feature = consensus_feature.reshape(N, C, -1).permute(0, 2, 1).contiguous()

        return consensus_feature, feat_pro_prediction   # N, l, C     N, 1, H, W
    ...
    def get_prompt(self, x, prompt, block_num, depth_num):
        feat = prompt[0] + prompt[2]
        prompt = prompt[1] + prompt[2]
        feat = torch.cat((feat, prompt), 2)
        # feat = prompt[1] + prompt[2] + prompt[0]
...
class mit_b0_vcp(MixVisionTransformerVCP):
    def __init__(self, **kwargs):
        super(mit_b0_vcp, self).__init__(
...
@BACKBONES.register_module()
class mit_b1_vcp(MixVisionTransformerVCP):
    def __init__(self, **kwargs):
        super(mit_b1_vcp, self).__init__(
...
@BACKBONES.register_module()
class mit_b2_vcp(MixVisionTransformerVCP):
    def __init__(self, **kwargs):
        super(mit_b2_vcp, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
...
@BACKBONES.register_module()
class mit_b3_vcp(MixVisionTransformerVCP):
    def __init__(self, **kwargs):
        super(mit_b3_vcp, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
...
@BACKBONES.register_module()
class mit_b4_vcp(MixVisionTransformerVCP):
    def __init__(self, **kwargs):
        super(mit_b4_vcp, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
...
@BACKBONES.register_module()
class mit_b5_vcp(MixVisionTransformerVCP):
    def __init__(self, **kwargs):
        super(mit_b5_vcp, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
