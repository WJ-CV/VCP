...
import yaml
from dataset_g3_test import Test_get_loader
import torch
...
def eval_psnr(test_loaders, model, testsets=None, test_root_dir=None):
    ...
    for testset in testsets:
        ...
        saved_root = os.path.join(root_path, testset) 
        ...
        for batch in pbar:
            ...
            final_prediction = score_prediction
            one_calss_nums = final_prediction.size()[0]
            for inum in range(one_calss_nums):
                img = final_prediction[inum, :, :, :].unsqueeze(0) 
    ...
    eval_loader = EvalDataset(saved_root, test_root_dir + testset + '/gt/')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt', default='./train_segformer_vcp_cosod/prompt_tuning.pth')
    ...
    for testset in testsets:
        test_loader = Test_get_loader(
            os.path.join(test_root_dir, testset, 'img/'), test_config['inp_size'], 1,
    ...
    if 'segformer' in config['model']['name']:
        ...
        checkpoint = load_checkpoint(model.encoder, config['model']['args']['encoder_mode']['pretrained'])
